...
proxies = {
    ...
    'https': 'https://145.239.85.58:9300',
}
...
while i < numbert:
    ...
    req_divar = requests.post(url=url_divar, json=json_divar, headers=random_head, proxies=proxies)
    print("1-divar", req_divar)
    time.sleep(sleep_time)

    # ارسال درخواست به url_snapp
    req_snapp = requests.post(url=url_snapp, json=json_snapp, headers=random_head, proxies=proxies)
    print("2-snapp", req_snapp)
    time.sleep(sleep_time)

    # ارسال درخواست به url_sheypoor
    req_sheypoor = requests.post(url=url_sheypoor, json=json_sheypoor, headers=random_head, proxies=proxies)
